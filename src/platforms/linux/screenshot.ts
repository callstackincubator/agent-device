...
async function resolveScreenshotTool(): Promise<{ tool: ScreenshotTool; display: 'wayland' | 'x11' }> {
  if (cachedTool) return cachedTool;

  if (isWayland()) {
    if (await whichCmd('grim')) { cachedTool = { tool: 'grim', display: 'wayland' }; return cachedTool; }
    if (await whichCmd('gnome-screenshot')) { cachedTool = { tool: 'gnome-screenshot', display: 'wayland' }; return cachedTool; }
    throw new AppError(
      'TOOL_MISSING',
  ...
  if (await whichCmd('scrot')) { cachedTool = { tool: 'scrot', display: 'x11' }; return cachedTool; }
  if (await whichCmd('import')) { cachedTool = { tool: 'import', display: 'x11' }; return cachedTool; }
  if (await whichCmd('gnome-screenshot')) { cachedTool = { tool: 'gnome-screenshot', display: 'x11' }; return cachedTool; }
  throw new AppError(
    'TOOL_MISSING',
