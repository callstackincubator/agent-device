...
async function resolveClipboardTool(): Promise<{ tool: ClipboardTool; display: 'wayland' | 'x11' }> {
  if (cachedTool) return cachedTool;

  if (isWayland()) {
    // wl-clipboard provides both wl-paste and wl-copy
    if (await whichCmd('wl-paste')) { cachedTool = { tool: 'wl-clipboard', display: 'wayland' }; return cachedTool; }
    throw new AppError(
      'TOOL_MISSING',
  ...
  if (await whichCmd('xclip')) { cachedTool = { tool: 'xclip', display: 'x11' }; return cachedTool; }
  if (await whichCmd('xsel')) { cachedTool = { tool: 'xsel', display: 'x11' }; return cachedTool; }
  throw new AppError(
    'TOOL_MISSING',
