...
import {
  withInteractionTimingResult,
  type InteractionTimingResult,
} from '../utils/interaction-timing.ts';
import {
  requireIntInRange,
      ...
      switch (command) {
        ...
        case 'press': {
          ...
          await runRepeatedSeries(count, intervalMs, async (index) => {
            ...
            if (holdMs > 0)
              interactionResult = await interactor.longPress(targetX, targetY, holdMs);
        ...
        case 'swipe': {
          ...
          await runRepeatedSeries(count, pauseMs, async (index) => {
            ...
            if (reverse)
              interactionResult = await interactor.swipe(x2, y2, x1, y1, effectiveDurationMs);
