...
const mockRunIosRunnerCommand = vi.fn<
  (device: DeviceInfo, command: RunnerCommand, opts?: unknown) => Promise<Record<string, unknown>>
>();

vi.mock('../../platforms/ios/runner-client.ts', async (importOriginal) => {
  const original = await importOriginal<typeof import('../../platforms/ios/runner-client.ts')>();
  return {
    ...original,
    runIosRunnerCommand: (...args: Parameters<typeof mockRunIosRunnerCommand>) =>
      mockRunIosRunnerCommand(...args),
  };
});

const { getInteractor, resolveAppleBackRunnerCommand, scrollIntoViewIosRunnerText } = await import(
  '../interactors.ts'
);
