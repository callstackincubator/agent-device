...
- HTTP JSON-RPC also exposes `agent_device.install_from_source` for typed host-side download/materialize/install flows. It accepts `{ platform, source: { kind: "url" | "path", ... }, session?, requestId? }` and returns normalized app identity (`packageName`/`bundleId`, `launchTarget`). It does not return host materialization paths because archive-backed installs are cleaned up before the RPC completes.
