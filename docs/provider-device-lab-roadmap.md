...
- [ ] Use that matrix for another deletion pass over mock-heavy handler tests.
...
- Device Lab coverage is broad but not yet mapped command-by-command.
- Remaining handler tests still contain large mocked suites, especially session, interaction, snapshot, and record/trace coverage.
