...
| --- | --- | --- | --- |
| `open`, `close`, `session_list`, `appstate` | Android lifecycle, iOS lifecycle, tvOS remote, macOS desktop, Linux desktop | invalid open args, session conflict/lock behavior, runtime hint edge cases, close shutdown edge cases | Review remaining happy paths in `session.test.ts` and `session-close-shutdown.test.ts`. |
...
| `keyboard` | Android status, iOS dismiss | keyboard state parsing, duplicate dumpsys fields, unsupported dismiss flows | Keep Android keyboard parser/dismiss edge tests. |
