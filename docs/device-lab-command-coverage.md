...
| `snapshot`, `screenshot` | Android snapshot/screenshot and provider failure normalization, iOS snapshot, macOS snapshot, Linux snapshot/screenshot, scoped Linux `@ref`/depth snapshots | snapshot processing, selector pruning, screenshot scaling/format edge cases, Android freshness retries | Delete only broad snapshot handler duplicates; keep narrow processing and freshness units. |
...
4. `src/daemon/handlers/__tests__/snapshot-handler.test.ts`: review broad snapshot success duplicates now that scoped snapshots are covered; keep freshness and processing edge tests.
...
- Broader record/trace and interaction scenarios before deleting non-trivial edge units.
