...
- Done: split pure snapshot state and visibility robustness tests into a mock-free `snapshot-capture.test.ts`, reducing `snapshot-handler.test.ts` to handler behavior, freshness, retry, and error policy.
- Done: moved Android perf related-action coverage into Device Lab by asserting the real lifecycle scenario reports recent `open` actions from the gfxinfo sample window; deleted the isolated mocked `session-perf.test.ts`.
- Done: moved trigger-event session recording/context coverage into the Android lifecycle scenario and narrowed `session-trigger.test.ts` to admission behavior.
- Done: split pure interaction flag and find argument parser matrices into mock-free helper suites, leaving the interaction/find handler tests focused on session behavior, selector routing, freshness, and edge cases.
- Still retained: Android freshness/collapse warnings, macOS menubar interaction guard/ref-promotion edges, wait routing, alert retry/error policy, diff invalid-kind/client-backed boundaries, recording touch visualization, off-screen failures, parser matrices, and pure snapshot state/visibility shaping.
- Next checkpoint: continue only where Device Lab can cover a plain success workflow; otherwise move pure parser/helper tests out of mock-heavy files and leave edge/state-machine tests in unit coverage.
...
- Apple raw tool/helper pressure is visible but not yet a mandate for more semantic sub-providers. The macOS helper calls are still one host-helper contract, while iOS simulator/device operations already have semantic `simctl` and `devicectl` runners. The latest provider-pressure review did not find a repeated Adapter intent that would justify another semantic Provider yet.
