...
- The screenshot flag pass is a useful architecture check. The remaining unavoidable places for a new screenshot flag should be the public CLI schema/help, the screenshot flag codec, the platform/backend implementation when behavior changes, and focused tests. CLI command handlers, typed client forwarding, daemon context creation, runtime screenshot dispatch, action recording, and replay script formatting should not each learn the flag independently.
