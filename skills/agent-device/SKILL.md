...
- **`logs start`** — Start streaming; requires an active app session (`open` first). Supported on iOS simulator, iOS device, and Android.
- **`logs stop`** — Stop streaming. Session `close` also stops logging.
...
- Retention knobs: `AGENT_DEVICE_APP_LOG_MAX_BYTES` and `AGENT_DEVICE_APP_LOG_MAX_FILES`.
- Optional write-time redaction patterns: `AGENT_DEVICE_APP_LOG_REDACT_PATTERNS` (comma-separated regex).

**Grepping app logs:** After getting the path from `logs path`, run `grep` (or `grep -E`) on that path so only matching lines enter context—keeping token use low.
...
- Use `-n` for line numbers. Use `-E` for extended regex so `|` in the pattern does not need escaping.
- Prefer targeted patterns (e.g. `Error`, `Exception`, or your app’s log tags) over reading the whole file.
