...
- In iOS sessions, use `open <app>` for simulator/device. `open <url>` works on both simulator and device. On devices, `http(s)://` URLs fall back to Safari; custom scheme URLs require an active app in the session.
