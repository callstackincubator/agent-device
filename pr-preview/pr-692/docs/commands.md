...
For persistent defaults and project-scoped CLI settings, see [Configuration](/agent-device/pr-preview/pr-692/docs/configuration.md).
...
- Run it after the simulator/device is booted and the app is installed, but before the first test command.
...
- Avoid broad `restore-keys` fallbacks for runner caches. Reusing runner artifacts across Agent Device or Xcode versions can restore stale `.xctestrun` products; `prepare ios-runner` already handles bad exact-cache artifacts and one stuck launch.
...
See [Replay & E2E](/agent-device/pr-preview/pr-692/docs/replay-e2e.md) for recording, Maestro compatibility, and CI workflow details.
...
See [Batching](/agent-device/pr-preview/pr-692/docs/batching.md) for payload format, response shape, and usage guidelines.
