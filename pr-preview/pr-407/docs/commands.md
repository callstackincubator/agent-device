...
For persistent defaults and project-scoped CLI settings, see [Configuration](/agent-device/pr-preview/pr-407/docs/configuration.md).
...
- `connect --remote-config <path>` is the canonical remote flow for sandbox agents. The remote profile supplies host, tenant/run scope, session, platform, lease backend, and Metro settings; `connect` allocates the lease, prepares Metro locally when needed, and stores non-secret connection state for normal follow-up commands.
- Command-line flags override connected defaults. Use them intentionally when switching session, platform, target, tenant, run, or lease scope.
- `metro prepare --remote-config <path>` remains available for advanced inspection and debugging. It prints JSON runtime hints to stdout, `--json` wraps them in the standard `{ success, data }` envelope, and `--runtime-file <path>` persists the same payload when callers need an artifact.
...
See [Replay & E2E (Experimental)](/agent-device/pr-preview/pr-407/docs/replay-e2e.md) for recording and CI workflow details.
...
See [Batching](/agent-device/pr-preview/pr-407/docs/batching.md) for payload format, response shape, and usage guidelines.
