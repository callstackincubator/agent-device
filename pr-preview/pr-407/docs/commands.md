...
- `connect --remote-config <path> --tenant <id> --run-id <id>` is the canonical remote flow for sandbox agents. The remote profile supplies host + Metro settings, `connect` allocates the lease, prepares Metro locally when needed, and stores non-secret connection state for normal follow-up commands.
...
agent-device connect --remote-config ./agent-device.remote.json --tenant acme --run-id run-123
...
- `--remote-config <path>` points to a remote workflow profile that captures stable host + Metro settings for `connect`.
...
- After `connect`, `snapshot`, `press`, `fill`, `screenshot`, and other normal commands reuse active connection state so agents do not repeat remote host/session/lease selectors inline.
