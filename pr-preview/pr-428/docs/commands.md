...
For persistent defaults and project-scoped CLI settings, see [Configuration](/agent-device/pr-preview/pr-428/docs/configuration.md).
...
See [Replay & E2E (Experimental)](/agent-device/pr-preview/pr-428/docs/replay-e2e.md) for recording and CI workflow details.
...
See [Batching](/agent-device/pr-preview/pr-428/docs/batching.md) for payload format, response shape, and usage guidelines.
...
- When `--overlay-refs` and `--max-size` are combined, the burned-in labels are resized with the screenshot, so very small max sizes can make labels harder to read.
