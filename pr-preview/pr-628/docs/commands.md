...
For persistent defaults and project-scoped CLI settings, see [Configuration](/agent-device/pr-preview/pr-628/docs/configuration.md).
...
See [Replay & E2E](/agent-device/pr-preview/pr-628/docs/replay-e2e.md) for recording, Maestro compatibility, and CI workflow details.
...
See [Batching](/agent-device/pr-preview/pr-628/docs/batching.md) for payload format, response shape, and usage guidelines.
...
agent-device settings clean-user-data
agent-device settings clean-user-data com.example.app
...
- `settings clean-user-data [app-id]` clears the active session app data, or the provided app id. Android uses `pm clear`, which removes SharedPreferences, databases, files, and cache. iOS simulator removes the app data container contents. iOS physical devices and macOS are unsupported.
