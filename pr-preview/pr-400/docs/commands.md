...
For persistent defaults and project-scoped CLI settings, see [Configuration](/agent-device/pr-preview/pr-400/docs/configuration.md).
...
Because default mobile snapshot output hides off-screen refs, prefer plain text from the off-screen summary when the target is not currently visible, such as `scrollintoview "Battery"`.
...
Use `scrollintoview @ref` only when you have a known raw/current ref or an interaction fails with an explicit off-screen-ref error; then retry with the returned `currentRef` or a fresh snapshot.
...
See [Replay & E2E (Experimental)](/agent-device/pr-preview/pr-400/docs/replay-e2e.md) for recording and CI workflow details.
...
See [Batching](/agent-device/pr-preview/pr-400/docs/batching.md) for payload format, response shape, and usage guidelines.
