...
- If a target only appears in an off-screen summary, use `scrollintoview "<label>"` or scroll and re-snapshot. Use `scrollintoview @ref` only with a known raw/current ref or after an explicit off-screen-ref failure.
