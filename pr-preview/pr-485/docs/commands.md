...
For persistent defaults and project-scoped CLI settings, see [Configuration](/agent-device/pr-preview/pr-485/docs/configuration.md).
...
See [Replay & E2E (Experimental)](/agent-device/pr-preview/pr-485/docs/replay-e2e.md) for recording and CI workflow details.
...
See [Batching](/agent-device/pr-preview/pr-485/docs/batching.md) for payload format, response shape, and usage guidelines.
...
- For Android and iOS sessions connected through a remote/cloud profile, `react-devtools` registers a lease-scoped companion tunnel to the sandbox-local DevTools daemon at `127.0.0.1:8097`. The remote service owns device-side routing and the CLI unregisters the companion when the command exits.
- Remote React DevTools assumes the React Native-bundled DevTools behavior in React Native 0.83+. Older browser/Chromium DevTools workflows are not assumed to exist inside remote sandboxes. Expo projects should be verified against the SDK's bundled React Native version before relying on this path; this release does not claim a separately verified Expo SDK version.
...
```json
{
  "daemonBaseUrl": "https://cloud.example.com/agent-device",
  ...
  "metroProjectRoot": ".",
  "metroProxyBaseUrl": "https://cloud.example.com"
}
```
...
- `AGENT_DEVICE_CLOUD_BASE_URL` should point at the cloud/control-plane API origin, not necessarily the dashboard origin. API-token setup links use `/api-keys` on that origin so the service can redirect users to the right dashboard page.
...
- `metroProxyBaseUrl` is the remote service origin. Do not prebuild `/api/metro/...` paths in the client profile; the CLI calls the cloud service endpoints itself.
- For cloud stock React Native iOS, the remote service descriptor supplies direct wildcard HTTPS Metro hints such as `<runtime>.metro.agent-device.dev:443`. The XCTest runner package is still used for runner-backed device commands, not for Metro reachability.
- Android keeps using service-provided runtime routes such as `/api/metro/runtimes/<runtimeId>/...`.
- `metroPublicBaseUrl` is only needed for direct/local bundle hints. Remote profiles can omit it and rely on `metroProxyBaseUrl`.
