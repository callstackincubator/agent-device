...
- Use `--daemon-auth-token <token>` (or `AGENT_DEVICE_DAEMON_AUTH_TOKEN`) when the remote daemon expects the shared daemon token over HTTP.
...
See [Replay & E2E (Experimental)](/agent-device/pr-preview/pr-189/docs/replay-e2e.md) for recording and CI workflow details.
...
See [Batching](/agent-device/pr-preview/pr-189/docs/batching.md) for payload format, response shape, and usage guidelines.
