...
For Node, Xcode, Android SDK, macOS, and iOS device prerequisites, see [Installation](/agent-device/pr-preview/pr-719/docs/installation.md).
...
For the broader positioning, supported targets, observability features, and how `agent-device` differs from scripted test frameworks, see [Introduction](/agent-device/pr-preview/pr-719/docs/introduction.md). For exact command groups and platform behavior, see [Commands](/agent-device/pr-preview/pr-719/docs/commands.md).

For the local execution model, permissions, artifacts, and sensitive data guidance, see [Security & Trust](/agent-device/pr-preview/pr-719/docs/security-trust.md).
