...
For persistent defaults and project-scoped CLI settings, see [Configuration](/agent-device/pr-preview/pr-286/docs/configuration.md).
...
- `is visible` checks whether the resolved element is present in the current visible snapshot viewport. It does not fail only because the exact node is not internally hittable when a visible ancestor carries the on-screen geometry.
...
See [Replay & E2E (Experimental)](/agent-device/pr-preview/pr-286/docs/replay-e2e.md) for recording and CI workflow details.
...
See [Batching](/agent-device/pr-preview/pr-286/docs/batching.md) for payload format, response shape, and usage guidelines.
