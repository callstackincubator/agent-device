...
For persistent defaults and project-scoped CLI settings, see [Configuration](/agent-device/pr-preview/pr-410/docs/configuration.md).
...
See [Replay & E2E (Experimental)](/agent-device/pr-preview/pr-410/docs/replay-e2e.md) for recording and CI workflow details.
...
See [Batching](/agent-device/pr-preview/pr-410/docs/batching.md) for payload format, response shape, and usage guidelines.
...
- `diff frames` summarizes transitions from a PNG frame directory or explicit PNG frame list. It prints a capped timeline with transition timing, keyframes, changed-region summaries, and optional OCR movement hints.
