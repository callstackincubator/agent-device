...
- For remote `connect --remote-config` sessions, see [Commands](/agent-device/pr-preview/pr-463/docs/commands.md#remote-metro-workflow).
- Use `--session <name>` to run multiple sessions in parallel.

For replay scripts and deterministic E2E guidance, see [Replay & E2E (Experimental)](/agent-device/pr-preview/pr-463/docs/replay-e2e.md).
