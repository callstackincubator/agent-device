...
For persistent defaults and project-scoped CLI settings, see [Configuration](/agent-device/pr-preview/pr-576/docs/configuration.md).
...
agent-device gesture pinch 2.0          # zoom in 2x (Apple simulator or macOS app session)
agent-device gesture pinch 0.5 200 400 # zoom out at coordinates (Apple simulator or macOS app session)
agent-device gesture rotate 35 200 420 # rotate app content (iOS simulator app session)
```
...
`gesture pinch` is supported on Apple simulators and macOS app sessions.
`gesture rotate` is supported on iOS simulator app sessions. Use `rotate` for device orientation.
...
See [Replay & E2E](/agent-device/pr-preview/pr-576/docs/replay-e2e.md) for recording, Maestro compatibility, and CI workflow details.
...
See [Batching](/agent-device/pr-preview/pr-576/docs/batching.md) for payload format, response shape, and usage guidelines.
