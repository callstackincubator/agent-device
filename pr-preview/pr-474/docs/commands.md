...
For persistent defaults and project-scoped CLI settings, see [Configuration](/agent-device/pr-preview/pr-474/docs/configuration.md).
...
See [Replay & E2E (Experimental)](/agent-device/pr-preview/pr-474/docs/replay-e2e.md) for recording and CI workflow details.
...
See [Batching](/agent-device/pr-preview/pr-474/docs/batching.md) for payload format, response shape, and usage guidelines.
...
- Without `--json`, `perf` prints a compact platform-independent frame-health summary when frame data is available, and points to `--json` when only CPU/memory data is available.
