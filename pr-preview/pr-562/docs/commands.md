...
For persistent defaults and project-scoped CLI settings, see [Configuration](/agent-device/pr-preview/pr-562/docs/configuration.md).
...
See [Replay & E2E](/agent-device/pr-preview/pr-562/docs/replay-e2e.md) for recording, Maestro compatibility, and CI workflow details.
...
See [Batching](/agent-device/pr-preview/pr-562/docs/batching.md) for payload format, response shape, and usage guidelines.
...
- On Android, use `alert wait`, `alert accept`, and `alert dismiss` for runtime permission prompts and native alerts. Do not use `settings permission` to answer a dialog already on screen; reserve it for setup or resetting permission state before a flow.
