...
For persistent defaults and project-scoped CLI settings, see [Configuration](/agent-device/pr-preview/pr-252/docs/configuration.md).
...
agent-device click @e1 --secondary          # compatibility alias
...
`click --button secondary` is the canonical desktop context-menu flow on macOS. `--secondary` remains supported as an alias.
...
See [Replay & E2E (Experimental)](/agent-device/pr-preview/pr-252/docs/replay-e2e.md) for recording and CI workflow details.
...
See [Batching](/agent-device/pr-preview/pr-252/docs/batching.md) for payload format, response shape, and usage guidelines.
