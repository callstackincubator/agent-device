...
For persistent defaults and project-scoped CLI settings, see [Configuration](/agent-device/pr-preview/pr-519/docs/configuration.md).
...
See [Replay & E2E (Experimental)](/agent-device/pr-preview/pr-519/docs/replay-e2e.md) for recording and CI workflow details.
...
See [Batching](/agent-device/pr-preview/pr-519/docs/batching.md) for payload format, response shape, and usage guidelines.
...
- For slow React Native app flows that need log markers, React profiles, network evidence, and a component performance report, start with `agent-device help react-native-performance`.
