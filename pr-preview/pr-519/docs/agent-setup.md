...
For Node, Xcode, Android SDK, macOS, and iOS device prerequisites, see [Installation](/agent-device/pr-preview/pr-519/docs/installation.md).
...
```text
Use agent-device only for app/device automation tasks. Before planning commands, run `agent-device --version` and read `agent-device help workflow`. For exploratory QA, read `agent-device help dogfood`. For logs, network, traces, or runtime failures, read `agent-device help debugging`. For React Native component trees, props/state/hooks, slow renders, or rerenders, read `agent-device help react-devtools`. For slow React Native app flows that need log markers, React profiles, network evidence, and a performance report, read `agent-device help react-native-performance`.
...
agent-device help react-native-performance
```
...
For the broader positioning, supported targets, observability features, and how `agent-device` differs from scripted test frameworks, see [Introduction](/agent-device/pr-preview/pr-519/docs/introduction.md). For exact command groups and platform behavior, see [Commands](/agent-device/pr-preview/pr-519/docs/commands.md).

For the local execution model, permissions, artifacts, and sensitive data guidance, see [Security & Trust](/agent-device/pr-preview/pr-519/docs/security-trust.md).
