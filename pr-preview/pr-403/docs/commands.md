...
- If `tesseract` is installed, `diff screenshot` also adds best-effort OCR text deltas, movement clusters, added/removed text candidates, and bbox size-change hints to the text and JSON output. OCR improves descriptions only; it does not change the pixel comparison or the diff PNG.
