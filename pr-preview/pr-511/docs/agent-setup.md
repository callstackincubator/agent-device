...
Some agent clients run commands in an environment that differs from the user's normal install shell. If the user installed `agent-device` globally but the agent cannot find it, have the agent run:

```bash
zsh -lic 'command -v agent-device'
```

Then use the returned absolute path for `--version`, `help workflow`, and subsequent commands.
...
For the broader positioning, supported targets, observability features, and how `agent-device` differs from scripted test frameworks, see [Introduction](/agent-device/pr-preview/pr-511/docs/introduction.md). For exact command groups and platform behavior, see [Commands](/agent-device/pr-preview/pr-511/docs/commands.md).

For the local execution model, permissions, artifacts, and sensitive data guidance, see [Security & Trust](/agent-device/pr-preview/pr-511/docs/security-trust.md).
